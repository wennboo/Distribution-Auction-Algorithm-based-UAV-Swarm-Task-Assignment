...
while(cnt<3):
    ...
    while(minadd<20):
        ...
        # print(af)
        if cnt==0:
            x_minaddall.append(minadd)
            y_sumall.append(sum)
            y_timeall.append(end-start)
        elif cnt==1:
            x_minaddline.append(minadd)
            y_sumline.append(sum)
            y_timeline.append(end-start)
        elif cnt==2:
            x_minaddrandom.append(minadd)
...
plt.plot(x_minaddall,y_timeall, linewidth=1,c='red',label="all")
plt.plot(x_minaddline,y_timeline, linewidth=1,c='yellow',label='line')
plt.plot(x_minaddrandom,y_timerandom, linewidth=1,c='blue',label='random')
plt.legend()
plt.show()